...
if __name__ == '__main__':
    ...
    for i, sample in tqdm(tfa_samples.iterrows()):
        ...
        # We also limt activation and inhibition constants from 0.1 to 10 x [C]
        ##################################################################################

        cofactors = [ 'atp_c', 'adp_c','amp_c',
                      'atp_m', 'adp_m', 
                      'nad_c','nad_m', 
                      'nadh_m', 'nadh_c', 
                      'coa_c', 'coa_m',
        ...
        # # # Some very useful plot for debugging the kinetics 

        # # Test jacbian
        # J = kmodel.jacobian_fun(fluxes,concentrations, params_population['0'])
        # variables = [kmodel.variables.iloc(i)[0] for i in kmodel.independent_variables_ix]
        # J = pd.DataFrame(J.todense(), columns = variables, index = variables)

        # f, ax = plt.subplots(1,1, figsize=(15,12))
        # sns.heatmap(J, ax=ax, cmap='viridis')
        # plt.savefig('test_J.png')

        # Ei = kmodel.independent_elasticity_fun(concentrations, params_population['0'])
        # flux_names = [r for r in kmodel.reactions]
        # Ei = pd.DataFrame(Ei.todense(), columns = variables, index = flux_names)

        # f, ax = plt.subplots(1,1, figsize=(15,12))
        # sns.heatmap(Ei, ax=ax, cmap='viridis')
        # plt.savefig('test_Ei.png')

        # M = modal_matrix(kmodel,concentrations,params_population['0'])

        # f, ax = plt.subplots(1,1, figsize=(20,20))
        # sns.heatmap(abs(M), ax=ax, cmap='viridis')
        # plt.tight_layout()
        # plt.savefig('test_M.png')


        # M = modal_matrix(kmodel,concentrations,params_population['0'], flux_modes=True,)

        # f, ax = plt.subplots(1,1, figsize=(20,20))
        # sns.heatmap(abs(M), ax=ax, cmap='viridis')
        # plt.tight_layout()
        # plt.savefig('test_M_fluxes.png')

        # dependent_weights =  kmodel.dependent_elasticity_fun.\
        #         get_dependent_weights(
        #                         concentration_vector=concentrations,
        #                         L0=kmodel.conservation_relation,
        #                         all_dependent_ix=kmodel.dependent_variables_ix,
        #                         all_independent_ix=kmodel.independent_variables_ix,
        #                     )
        
        # dep_variables = [kmodel.variables.iloc(i)[0] for i in kmodel.dependent_variables_ix]
        # Q = pd.DataFrame(dependent_weights.todense() , columns=variables, index=dep_variables)

        # f, ax = plt.subplots(1,1, figsize=(15,12))
        # sns.heatmap(Q, ax=ax, cmap='viridis')
    ...
    plt.savefig(tfa_sample_file.replace(".csv", "_eigenvalue_histogram.svg"))
